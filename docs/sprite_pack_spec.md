...
对于需要占据多格或特殊尺寸的贴图（如 48×64 的松树），可在 `tiles.json` 中配置渲染参数。

设计借鉴 Cataclysm-DDA 的 `sprite_offset_x/y` 像素偏移方案，比锚点比例更直观。

### 文件位置

按优先级查找：
1. **classpath**：`/gfx/sprites/tile/tiles.json`（打包进 JAR）
2. **filesystem**：`sprites/tile/tiles.json`（项目根目录，开发用）
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `width` | double | 1.0 | 渲染宽度（瓦片单位）。1.0 = 1 格（32px），1.5 = 1.5 格（48px） |
| `height` | double | 1.0 | 渲染高度（瓦片单位）。1.0 = 1 格（32px），2.0 = 2 格（64px） |
| `offsetX` | int | 0 | 像素偏移 X。正值向右，负值向左 |
| `offsetY` | int | 0 | 像素偏移 Y。正值向下，负值向上 |

### 偏移量定位示意

偏移量是精灵相对默认位置（左上角对齐瓦片左上角）的**像素位移**：
...
| 场景 | width | height | offsetX | offsetY | 效果 |
|------|-------|--------|---------|---------|------|
| 默认（贴齐瓦片） | 1.0 | 1.0 | 0 | 0 | 精灵左上角 = 瓦片左上角 |
| 松树（48×64 居中扎根） | 1.5 | 2.0 | 0 | -16 | 水平居中，底部延伸 0.5 格 |
| 向左偏移的招牌 | 1.0 | 1.0 | -8 | 0 | 精灵向左偏移 8 像素 |
| 向上生长的藤蔓 | 1.0 | 2.0 | 0 | -32 | 精灵向上延伸 1 格 |

**偏移量 vs 锚点的对比**：

| 概念 | 锚点（anchorX/Y） | 偏移量（offsetX/Y） |
|------|-------------------|---------------------|
| 单位 | 精灵自身比例（0.0~1.0） | 像素 |
| 心智模型 | "精灵上哪个点对齐瓦片位置" | "精灵相对默认位置移动多少像素" |
| 计算示例 | 48×64 居中扎根：anchorX=0, anchorY=0.25 | 48×64 居中扎根：offsetX=0, offsetY=-16 |
| 直观性 | 需要心算 `0.25 × 64 = 16px` | 直接看到 `-16` 表示上移 16px |

**松树（48×64）的 offsetY=-16 推导**：
- 精灵高度 64px，瓦片高度 32px
- 默认位置：精灵顶部 = 瓦片顶部（`drawY = tileScreenY`）
- 希望：精灵向下延伸 0.5 格（"扎根"到下方瓦片）
- 即精灵底部 = `tileScreenY + 48`，比瓦片底部（`tileScreenY + 32`）多 16px
- 因此需要上移 16px：`offsetY = -16`

### 多瓦片去重

当 `width > 1.0` 或 `height > 1.0` 时，精灵会被标记为多瓦片精灵。覆盖层渲染会自动：
1. 计算精灵占据的所有瓦片位置
2. 标记这些位置，避免相邻瓦片重复绘制同一精灵

### 注意事项

- 未配置的地形默认 `width=1.0, height=1.0, offsetX=0, offsetY=0`（单格渲染）
- PNG 原始尺寸无需与目标尺寸一致，会按最近邻插值缩放到 `width × tileSize` × `height × tileSize` 像素
- 仅对 PNG 加载生效，程序化回退（PixelArt）不受影响（保持 32×32）
...
| `TileSpriteData` | 地形贴图加载与程序化生成（含 tiles.json 配置解析） |
| `CreatureSpriteData` | 生物贴图加载与程序化生成 |
| `ItemSpriteData` | 物品贴图（规划中） |
| `Sprite` | 贴图数据载体（id + BufferedImage + 尺寸 + 锚点） |
